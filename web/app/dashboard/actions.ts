...
if (!process.env.OPENSEARCH_URL) {
  throw new Error("OPENSEARCH_URL is required. Add it to your .env file (see web/.env.example).")
}
const OS = process.env.OPENSEARCH_URL
const TRACES_INDEX = "ss4o_traces-otel-aplicacao-example"
const LOGS_INDEX = "docker-logs"
const SERVICE_PREFIX = "aplication-exemple-"
...
function serviceFilter(serviceName?: string): object[] {
  ...
  // sem serviço específico → restringe ao prefixo aplicacao-exemplo-*
...
export async function getServices(): Promise<string[]> {
  ...
  try {
    ...
    return data.aggregations?.services?.buckets?.map((b: { key: string }) => b.key) ?? []
  } catch {
    return []
  }
}

export async function getServiceCapabilities(serviceName: string): Promise<ServiceCapabilities> {
  const body = {
    size: 0,
    query: {
      bool: {
...
export async function getKpis(from = "now-1h", to = "now", serviceName?: string): Promise<KpiData> {
  const body = {
    size: 0,
    query: {
      bool: {
        must: [{ term: { "kind.keyword": "Server" } }],
        filter: [{ range: { "@timestamp": timeRange(from, to) } }, ...serviceFilter(serviceName)],
      },
    },
    aggs: {
      total: { value_count: { field: "@timestamp" } },
      errors: { filter: { range: { "attributes.http.status_code": { gte: 400 } } } },
      avg_latency: { avg: { field: "durationInMillis" } },
      percentiles: { percentiles: { field: "durationInMillis", percents: [95] } },
      users_created: {
        filter: {
          bool: {
            must: [
  ...
  try {
    ...
    return {
      ...
      appointmentsCreated: data.aggregations?.appointments_created?.doc_count ?? 0,
      appointmentsDeleted: data.aggregations?.appointments_deleted?.doc_count ?? 0,
      appointmentsUpdated: data.aggregations?.appointments_updated?.doc_count ?? 0,
      appointmentsViewed: data.aggregations?.appointments_viewed?.doc_count ?? 0,
    }
  } catch {
    return {
      totalRequests: 0, totalErrors: 0, avgLatencyMs: 0, p95LatencyMs: 0, errorRate: 0,
      usersCreated: 0, usersDeleted: 0, usersUpdated: 0, usersViewed: 0,
      appointmentsCreated: 0, appointmentsDeleted: 0, appointmentsUpdated: 0, appointmentsViewed: 0,
    }
  }
}
...
function autoInterval(from: string, to: string): string {
  ...
  try {
    const diffMs = new Date(to === "now" ? Date.now() : to).getTime() - new Date(from).getTime()
...
export async function getRequestsOverTime(from = "now-1h", to = "now", serviceName?: string): Promise<TimePoint[]> {
  const body = {
    size: 0,
    query: {
      bool: {
  ...
  try {
    ...
    return buckets.map((b: Record<string, unknown>) => {
      const point: TimePoint = { time: formatBucketTime(b.key_as_string as string, from) }
      const services = (b.by_service as { buckets: Array<{ key: string; doc_count: number }> })?.buckets ?? []
...
export async function getLatencyOverTime(from = "now-1h", to = "now", serviceName?: string): Promise<TimePoint[]> {
  const body = {
    size: 0,
    query: {
      bool: {
        must: [{ term: { "kind.keyword": "Server" } }, { exists: { field: "durationInMillis" } }],
        filter: [{ range: { "@timestamp": timeRange(from, to) } }, ...serviceFilter(serviceName)],
      },
    },
    aggs: {
      over_time: {
        ...
        aggs: {
          latency: { percentiles: { field: "durationInMillis", percents: [50, 95, 99] } },
        },
      },
    },
  }
  ...
  try {
    ...
    const buckets = data.aggregations?.over_time?.buckets ?? []
    return buckets.map((b: Record<string, unknown>) => {
      const values = (b.latency as { values: Record<string, number | null> })?.values ?? {}
      return {
...
export async function getStatusOverTime(from = "now-1h", to = "now", serviceName?: string): Promise<TimePoint[]> {
  const body = {
    size: 0,
    query: {
      bool: {
        must: [{ term: { "kind.keyword": "Server" } }, { exists: { field: "attributes.http.status_code" } }],
        filter: [{ range: { "@timestamp": timeRange(from, to) } }, ...serviceFilter(serviceName)],
      },
    },
    aggs: {
      over_time: {
        ...
        aggs: {
          success: { filter: { range: { "attributes.http.status_code": { gte: 200, lt: 400 } } } },
          client_errors: { filter: { range: { "attributes.http.status_code": { gte: 400, lt: 500 } } } },
          server_errors: { filter: { range: { "attributes.http.status_code": { gte: 500 } } } },
        },
      },
    },
  }
...
export async function getLogCountOverTime(from = "now-1h", to = "now"): Promise<TimePoint[]> {
  const body = {
    size: 0,
  ...
  try {
    ...
    return buckets.map((b: Record<string, unknown>) => {
      ...
      const containers = (b.by_container as { buckets: Array<{ key: string; doc_count: number }> })?.buckets ?? []
...
export async function getThroughputByMethod(from = "now-1h", to = "now", serviceName?: string): Promise<TimePoint[]> {
  const body = {
    size: 0,
    query: {
      bool: {
        must: [{ term: { "kind.keyword": "Server" } }, { exists: { field: "attributes.http.method" } }],
        filter: [{ range: { "@timestamp": timeRange(from, to) } }, ...serviceFilter(serviceName)],
      },
    },
    aggs: {
      over_time: {
        ...
        aggs: {
          by_method: { terms: { field: "attributes.http.method.keyword", size: 10 } },
        },
      },
    },
  }
  ...
  try {
    ...
    return buckets.map((b: Record<string, unknown>) => {
      const point: TimePoint = { time: formatBucketTime(b.key_as_string as string, from) }
      const methods = (b.by_method as { buckets: Array<{ key: string; doc_count: number }> })?.buckets ?? []
...
export async function getLatencyByRouteOverTime(from = "now-1h", to = "now", serviceName?: string): Promise<TimePoint[]> {
  const body = {
    size: 0,
    query: {
      bool: {
        must: [{ term: { "kind.keyword": "Server" } }, { exists: { field: "attributes.http.target" } }],
        filter: [{ range: { "@timestamp": timeRange(from, to) } }, ...serviceFilter(serviceName)],
      },
    },
    aggs: {
      top_routes: {
        terms: { field: "attributes.http.target.keyword", size: 5, order: { _count: "desc" } },
        aggs: {
          over_time: {
            date_histogram: {
              field: "@timestamp",
  ...
  try {
    const data = await osSearch(TRACES_INDEX, body)
    const routeBuckets: Array<{ key: string; over_time: { buckets: Array<{ key_as_string: string; avg_latency: { value: number | null } }> } }> =
...
export async function getLatencyByRoute(from = "now-1h", to = "now", serviceName?: string): Promise<RouteLatency[]> {
  const body = {
    size: 0,
    query: {
      bool: {
    ...
    aggs: {
      by_route: {
        terms: { field: "attributes.http.target.keyword", size: 15, order: { _count: "desc" } },
        aggs: {
  ...
  try {
    const data = await osSearch(TRACES_INDEX, body)
    const buckets: Array<{ key: string; doc_count: number; p95: { values: Record<string, number | null> } }> =
...
export async function getOpDistribution(from = "now-1h", to = "now", serviceName?: string): Promise<OpEntry[]> {
  const body = {
    size: 0,
    query: {
      bool: {
    ...
    aggs: {
      by_op: { terms: { field: "name.keyword", size: 20, order: { _count: "desc" } } },
    },
  }

  try {
    const data = await osSearch(TRACES_INDEX, body)
    const buckets: Array<{ key: string; doc_count: number }> = data.aggregations?.by_op?.buckets ?? []
...
export async function getTopEndpoints(from = "now-1h", to = "now", serviceName?: string): Promise<EndpointRow[]> {
  const body = {
    size: 0,
    query: {
      bool: {
    ...
    aggs: {
      by_endpoint: {
        terms: { field: "attributes.http.target.keyword", size: 20, order: { _count: "desc" } },
        aggs: {
          avg_latency: { avg: { field: "durationInMillis" } },
          p95: { percentiles: { field: "durationInMillis", percents: [95] } },
        },
...
export async function getAppointmentsOverTime(from = "now-1h", to = "now", serviceName?: string): Promise<TimePoint[]> {
  const body = {
    size: 0,
    query: {
      bool: {
    ...
    aggs: {
      over_time: {
        ...
        aggs: {
          ...
          viewed:  { filter: { term: { "name.keyword": "GET /appointments/:id" } } },
          updated: { filter: { term: { "name.keyword": "PUT /appointments/:id" } } },
          deleted: { filter: { term: { "name.keyword": "DELETE /appointments/:id" } } },
        },
      },
    },
  }
