...
function uniqueEmail() {
  return `bp_vu${__VU}_i${__ITER}_${Date.now()}@break.com`;
}
function futureDate() {
  ...
  if (roll < 0.25) {
    ...
    if (res.status === 201) {
      ...
      try {
        ...
        if (b.id) {
          userIds.push(b.id);
          if (userIds.length > 200) userIds.shift();
        }
      } catch {
        /* ignore */
      }
    ...
    const res = http.post(
      `${API_V1}/appointments`,
      JSON.stringify({
        title: `BP Appt VU${__VU} IT${__ITER}`,
    ...
    if (res.status === 201) {
      ...
      try {
        ...
        if (b.id) {
          apptIds.push(b.id);
          if (apptIds.length > 200) apptIds.shift();
        }
      } catch {
        /* ignore */
      }
    ...
    if (res.status === 200) {
      ...
      errorCount.add(1);
      errorRate.add(1);
      console.error(`[5xx] GET /users/${id} -> ${res.status}`);
    } else {
      errorRate.add(0);
    }
    ...
    if (res.status === 200) {
      ...
      errorCount.add(1);
      errorRate.add(1);
      console.error(`[5xx] GET /appointments/${id} -> ${res.status}`);
    } else {
      errorRate.add(0);
    }
...
export function handleSummary(data) {
  ...
  if (errNum === 0)
    console.log("[RESULTADO] Sistema aguentou o teto (150 VUs) sem erros.");
  else if (errNum < 1)
    console.log("[RESULTADO] Sistema chegou proximo ao limite (<1% erros).");
  else if (errNum < 5)
    console.log("[RESULTADO] Degradacao detectada (1-5% erros).");
  else if (errNum < 20)
    console.log(
      "[RESULTADO] RUPTURA ENCONTRADA: sistema falhou significativamente.",
    );
